...
const MODEL_LABELS: Record<string, string> = {
  "texbat-xgb-v1": "XGBoost — L1 Signal (TEXBAT)",
  "aissou-xgb-bin-v1": "XGBoost — L2 Channel (Aissou)",
  "aissou-xgb-binary-v1": "XGBoost — L2 Channel (Aissou)",
};
    ...
    <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 lg:w-0 border-r border-slate-800 min-h-0 overflow-hidden">
        ...
        <div className="flex-none border-b border-slate-800 bg-slate-900/60 px-4 py-2 flex flex-wrap items-center gap-5 text-xs font-mono">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] tracking-wider uppercase text-slate-500">Altitude</span>
            <span className="tabular-nums text-slate-300">{fmtAlt(tick.position.alt)}</span>
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] tracking-wider uppercase text-slate-500">Heading</span>
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] tracking-wider uppercase text-slate-500">Attack</span>
            ...
            {(["L1", "L2"] as const).map((layer) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="font-mono text-[9px] text-slate-500">raw {s.raw.toFixed(4)} / thr {s.threshold.toFixed(2)}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-3 p-4">
          ...
          <div className="border border-slate-800 bg-slate-900/40 rounded-sm p-3 flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
              ...
              <Metric label="Dominant layer" value={dom} />
              <Metric label="L1 ratio" value={`${tick.scores.L1.ratio.toFixed(3)}×`} />
              <Metric label="L2 ratio" value={`${tick.scores.L2.ratio.toFixed(3)}×`} />
              <Metric label="L1 raw prob." value={tick.scores.L1.raw.toFixed(4)} />
              <Metric label="L2 raw prob." value={tick.scores.L2.raw.toFixed(4)} />
              <Metric label="L1 model" value={tick.scores.L1.model_version} small />
              <Metric label="L2 model" value={tick.scores.L2.model_version} small />
              <Metric label="Inference XGB" value={`${tick.inference_ms.xgboost.toFixed(1)} ms`} />
              <Metric label="Is attack" value={tick.is_attack ? "TAK ⚠" : "nie"} />
            </div>
          </div>
